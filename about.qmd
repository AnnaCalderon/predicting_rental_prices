...
```{r plot-aesthetics}
#| include: false
# ---- Global plot aesthetics (brand style) ----
theme_set(theme_minimal(base_size = 13))

# Brand palette
brand_cols <- list(
  blue_fill = "#4C78A8",  blue_line = "#3B5C80",
  orange_fill = "#F58518", orange_line = "#C56A13",
  grey_grid = "#E9ECEF", grey_text = "#666666"
)

# For easy reusable styling
theme_brand <- function() {
  theme_minimal(base_size = 13) +
    theme(
      panel.grid.minor = element_blank(),
      panel.grid.major.y = element_blank(),
      panel.grid.major.x = element_line(color = brand_cols$grey_grid),
      plot.title = element_text(face = "bold", size = 15),
      plot.subtitle = element_text(size = 11, color = brand_cols$grey_text),
      legend.position = "top",
...
update_geom_defaults("point", list(alpha = 0.7, color = brand_cols$blue_line))
update_geom_defaults("bar",   list(fill = brand_cols$blue_fill, alpha = 0.35, color = NA))
...
# EDA (brand-styled with centered, slightly smaller titles)
p1 <- ggplot(train, aes(price)) +
  geom_histogram(bins = 50, fill = brand_cols$blue_fill, alpha = 0.6) +
...
p2 <- ggplot(train, aes(log1p(price))) +
  geom_histogram(bins = 50, fill = brand_cols$orange_fill, alpha = 0.6) +
  labs(title = "Log-Price Distribution",
       x = "log1p(Price)", y = "Count") +
  theme_brand() +
  theme(plot.title = element_text(hjust = 0.5, size = 13))
...
p3 <- train %>%
  ...
  geom_boxplot(fill = brand_cols$blue_fill, alpha = 0.6) +
...
p4 <- ggplot(train, aes(x = accommodates, y = price)) +
  geom_point(size = 1, color = brand_cols$blue_line) +
  geom_smooth(se = FALSE, linewidth = 1, color = brand_cols$orange_line) +
  scale_y_continuous(labels = fmt_dollar) +
  labs(title = "Price vs. Accommodates",
       x = "Accommodates", y = "Price (USD)") +
  theme_brand() +
  theme(plot.title = element_text(hjust = 0.5, size = 13))
...
pB <- ggplot(best_preds, aes(x = pred, y = resid)) +
  geom_point(alpha = 0.25, size = 0.9, color = brand_cols$blue_line) +
  ...
  theme_brand() + theme(plot.title = element_text(hjust = 0.5))

# C) Residuals Q–Q Plot
pC <- ggplot(best_preds, aes(sample = resid)) +
  stat_qq(alpha = 0.5, size = 1, color = brand_cols$blue_line) +
  stat_qq_line(color = brand_cols$orange_line, linewidth = 0.9) +
...
bottom_row <- (pB | sp | pC) + patchwork::plot_layout(widths = c(1, 0.06, 1))
(pA) / bottom_row + patchwork::plot_layout(heights = c(1.35, 1))
...
# ================================================================
#   FEATURE IMPORTANCE (RF / XGB) or COEFFICIENTS (Elastic Net)
#   with short, human-friendly labels
# ================================================================

# ---- human-friendly renaming function ----
pretty_name <- function(x) {
  x <- tolower(x)
  x <- stringr::str_replace_all(x, "_", " ")

  replacements <- c(
    "neighbourhood group cleansed" = "Neighborhood",
    "zipcode"                      = "Zip Code",
    "zip price median"             = "Zip Median Price",
    "zip price mean"               = "Zip Mean Price",
    "count amenities"              = "Amenities Count",
    "review score 5"               = "Review Rating (0-5)",
  ...
  for (k in names(replacements)) {
    x <- stringr::str_replace_all(x, k, replacements[k])
  }
...
# ---- Show TOP FEATURES for the winning model ----
if (best_name == "RF") {

  # caret->ranger importance
  vi <- caret::varImp(best_fit)$importance %>%
    tibble::rownames_to_column("feature") %>%
    dplyr::arrange(dplyr::desc(Overall)) %>%
    dplyr::slice(1:15) %>%
    dplyr::mutate(label = pretty_name(feature))

  ggplot(vi, aes(x = reorder(label, Overall), y = Overall)) +
    geom_col(fill = brand_cols$blue_fill, alpha = 0.9) +
    coord_flip() +
    labs(title = "Top Features — Random Forest", x = "", y = "Importance") +
    theme_brand()
  ...
  ggplot(imp, aes(x = reorder(label, Gain), y = Gain)) +
    geom_col(fill = brand_cols$orange_fill, alpha = 0.9) +
    coord_flip() +
    labs(title = "Top Features — XGBoost", x = "", y = "Gain") +
    theme_brand()
  ...
  ggplot(en_coefs, aes(x = reorder(label, estimate), y = estimate)) +
    geom_col(fill = brand_cols$blue_fill, alpha = 0.9) +
    coord_flip() +
    labs(title = "Top Features — Elastic Net", x = "", y = "Coefficient") +
    theme_brand()
}
```
...
ggplot(analysisData, aes(price)) +
  geom_histogram(bins = 60, fill = "#4C78A8", alpha = 0.6) +
...
# Pretty density: Actual vs Predicted Nightly Prices
library(dplyr)
library(tidyr)
library(ggplot2)
library(scales)

# --- assumes best_preds with columns: price (actual), pred (predicted) ---
stopifnot(all(c("price","pred") %in% names(best_preds)))

# Focus the view by trimming extreme tails (keeps plot readable as a thumbnail)
x_max <- quantile(c(best_preds$price, best_preds$pred), 0.99, na.rm = TRUE)

plot_df <- best_preds |>
  transmute(Actual = price, Predicted = pred) |>
  pivot_longer(everything(), names_to = "Series", values_to = "value") |>
  filter(!is.na(value), value >= 0, value <= x_max)
...
cols_fill  <- c("Actual" = "#4C78A8", "Predicted" = "#F58518")
cols_line  <- c("Actual" = "#3B5C80", "Predicted" = "#C56A13")

p <- ggplot(plot_df, aes(x = value, fill = Series, color = Series)) +
  geom_density(alpha = 0.28, linewidth = 1.0, adjust = 1.0) +
  geom_vline(data = med_df, aes(xintercept = med, color = Series),
  ...
  labs(
    title = "Nightly Airbnb Prices — Actual vs Predicted",
    subtitle = "Predicted distribution closely follows the observed market (medians dashed)",
    x = "Nightly Price (USD)", y = NULL
  ) +
...
# High-res exports (perfect for a portfolio card)
ggsave("airbnb_price_density_pretty.png", p, width = 9, height = 5.2, dpi = 320)
ggsave("airbnb_price_density_pretty.svg", p, width = 9, height = 5.2)
...
# Make sure best_preds exists: it should contain columns price (actual) and pred (predicted)

ggplot(best_preds, aes(x = price, y = pred)) +
  geom_hex(bins = 35, show.legend = TRUE) +
  scale_fill_viridis_c(option = "C", name = "Count") +
  geom_abline(slope = 1, intercept = 0, linetype = 2, linewidth = 0.8, color = "white") +
  coord_equal() +
  scale_x_continuous(labels = dollar_format(accuracy = 1)) +
  scale_y_continuous(labels = dollar_format(accuracy = 1)) +
  labs(
    title = "Predicted vs Actual Nightly Airbnb Prices (NYC)",
    x = "Actual Price",
    y = "Predicted Price"
  ) +
  theme_minimal(base_size = 13) +
...
ggsave("airbnb_price_prediction_plot.png", width = 8, height = 6, dpi = 300)
```
